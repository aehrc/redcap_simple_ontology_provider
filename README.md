...
- ***BREAKING: a code's leading/trailing whitespace is now trimmed*** - Before priority-codes, a code's whitespace
  ...
  code (easy to introduce pasting from a spreadsheet, e.g. `C1 |Weekly`) was harmless. `Priority Codes` and
  `@HIDECHOICE` are the first things that need a code typed into one setting to match a code parsed from an
  entirely separate `Values` list, and that's where an accidental whitespace difference between the two silently
  prevents the match. `Values` codes for all three formats (`list`, `bar`, `json`) are now trimmed, so that
  cross-referencing is reliable.
  **If any existing category's `Values` happens to have a code with such incidental whitespace, and a record has
  ...
  configured label after upgrading (the code-to-label lookup is now keyed by the trimmed code). If you rely on
